...
Each model the best performing word embeddings configuration has its result in bold.
 }
\label{tab:representationOLD}
\centering
...
\begin{table}[h]
	...
	\label{tab:representationwithCA}
	\begin{tabular}{l|ccccc}
		\hline\noalign{\smallskip}
		\multirow{2}{*}{Models}		& \multicolumn{5}{ c }{Text representation}       \\ 
		& Non-static	& CA static		& ES static		& CA non-static	& ES non-static	\\
...
\begin{table}[h]
\footnotesize
\caption{Text representation study comparing 10-fold cross validation results over the development set in terms of percentage of $F_{1-macro}$ score. The pre-processing setting was fixed at RM+EM.}
...
\begin{table}[h]
	...
	\begin{tabular}{l|ccc}
		\hline\noalign{\smallskip}
		\multirow{2}{*}{Models}		& \multicolumn{3}{ c }{Text representation}       \\ 
...
For the system's submission, participants were allowed to send more than a model till a maximum of 5 possible runs, \cref{tab:results} reports our best performing systems at the COSET shared task.
