...
b_cnn_lstm_score, b_cnn_lstm_acc = b_cnn_lstm.run(metrics=[coset.fbeta_score], max_features=15000, maxlen=50,
...
cnn_score, cnn_acc = cnn.run(metrics=[coset.fbeta_score], max_features=15000, maxlen=50, batch_size=32,
                             embedding_dims=50, filters=250,
                             kernel_size=3,
                             hidden_dims=250, epochs=5)

b_lstm = BidirectionalLSTMModel(data_function=coset.load_data)
b_lstm_score, b_lstm_acc = b_lstm.run(metrics=[coset.fbeta_score], max_features=15000, maxlen=50, batch_size=32,
                                      epochs=3)

fast_text = FastTextModel(data_function=coset.load_data)
fast_text_score, fast_text_acc = fast_text.run(metrics=[coset.fbeta_score], max_features=15000, maxlen=50,
                                               ngram_range=4, embedding_dims=50,
                                               batch_size=32, epochs=5)

lstm = LSTMModel(data_function=coset.load_data)
lstm_score, lstm_acc = lstm.run(metrics=[coset.fbeta_score], max_features=15000, maxlen=50, batch_size=32, epochs=3)
