...
def run(cleaning_function):
    ...
    lstm.run(metrics=[metrics.fbeta_score], maxlen=max_len, language=language, clean_tweets=cleaning_function,
             batch_size=32,
    ...
    cnn.run(metrics=[metrics.fbeta_score], maxlen=max_len,
            ...
            kernel_size=3, dropout=0.2,clean_tweets=cleaning_function,
    ...
    b_lstm.run(metrics=[metrics.fbeta_score], max_len=max_len,
               batch_size=32, recurrent_units=64, dropout=0.1, language=language,clean_tweets=cleaning_function,
               epochs=2)
    ...
    cnn_lstm.run(metrics=[metrics.fbeta_score], maxlen=max_len, kernel_size=5, dropout=0.25, strides=1,
                 language=language,clean_tweets=cleaning_function,
    ...
    fast_text.run(metrics=[metrics.fbeta_score], maxlen=max_len,
                  ngram_range=2, hidden_dims=100, language=language,clean_tweets=cleaning_function,
    ...
    kim.run(metrics=[metrics.fbeta_score], maxlen=max_len,
            batch_size=32, strides=1, filters=150, language=language,clean_tweets=cleaning_function,
    ...
    with open("../stance_es-" + strftime("%Y%m%d_%H%M%S", gmtime()) + ".log", 'w') as outcsv:
